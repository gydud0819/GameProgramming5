...
private:
	...
	*/
	...
	void heapifyUp(Node* node)		// 새로 추가된 노드가 저장되어있고 노드의 부모 노드를 포인터로 가리키고 있음. node->parrentNode->value node->value
	{
		...
		while (node->value > node->parrentNode->value /*&& node->parrentNode*/)		// 변경하기 위한 조건
		{
			if (node->parrentNode != nullptr)
			{
				break;
			}
...
public:
	...
	void Insert(int value)
	{
		...
		while (!q.empty())	// 데이터가 아무것도 없을때까지 while로 계속 돌린다. q 데이터가있을때만 돌린다 
		{
			...
			q.pop();
			if (!currentNode->leftnode)		// 현재 노드에 데이터가 없으면 왼쪽 노드에 넣기
			{
...
int main()
{
	...
	for (int i = 0; i < 2; i++)
	{
		kitValue = mheap.exractMax();		// 2번째로 큰 수 kitValue 저장
	}
